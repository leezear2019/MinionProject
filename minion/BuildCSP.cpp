...
void BuildCSP(StateObj* stateObj, CSPInstance& instance)
{
  ...
  // Resize room in vector - no necessary but more efficent.
...
void SolveCSP(StateObj* stateObj, CSPInstance& instance, SearchMethod args)
{
  ...
  double diffT=getState(stateObj).getOldTimer().maybePrintFinaltimestepStore(cout, "Solve Time: ", "SolveTime", getTableOut(), !getOptions(stateObj).silent);
  int totalNodes= getState(stateObj).getNodeCount();
  ...
  ofile<<getOptions(stateObj).instance_name<<" "<<diffT<<" "<<totalNodes<<" "<<getState(stateObj).getSolutionCount()<<endl;
  ofile.close();
