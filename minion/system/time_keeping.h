...
double printTimestepWithoutReset(Stream& sout, Output_Type t, const char* time_name)
{
  if(t != Output_Always && t != output)
    return 0;
  sout << time_name << get_cpu_time() - _last_check_time << endl;
  return get_cpu_time() - _last_check_time;
}

template<typename Stream>
double maybePrintTimestepStore(Stream& sout, Output_Type t, const char* time_name, const char* store_name, TableOut & tableout, bool toprint)
{
  if(t != Output_Always && t != output)
    return 0;
...
template<typename Stream>
double maybePrintFinaltimestepStore(Stream& sout, const char* time_name, const char* store_name, TableOut & tableout, bool toprint)
{
  ...
  double diffT=maybePrintTimestepStore(sout, Output_Always, time_name, store_name, tableout, toprint);
  if(toprint)
  {
    sout << "Total Time: " << end_cpu_time - _internal_cpu_start_time << endl;
    //diffT=end_cpu_time - _internal_cpu_start_time;
