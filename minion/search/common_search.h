  ...
  void print_solution(StateObj* stateObj, Stream& sout, const PrintMatrix& print_matrix)
  {
    ...
    // TODO : Make this more easily changable.
      if (!getOptions(stateObj).silent)
      {
        ...
        double diffT=getState(stateObj).getOldTimer().printTimestepWithoutReset(sout, Output_Always, "Time:");
        ...
        ofile<<getOptions(stateObj).instance_name<<" "<<diffT<<" "<<getState(stateObj).getSolutionCount()<<endl;
        ofile.close();*/
      }
